    ...
    >
      ...
      <div className="px-5 py-6 border-b border-white/[0.06]">
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${ROLE_GRADIENT[user.role] ?? "from-indigo-500 to-violet-500"} flex items-center justify-center shadow-lg`}>
            <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <div>
            <p className="text-sm font-bold text-white tracking-tight">Sietrix CRM</p>
        ...
        {visible.map((item) => {
          ...
          return (
            <Link
              ...
              style={
                active
                  ? { borderLeft: "2px solid rgba(129,140,248,0.8)" }
      ...
      <div className="px-4 py-4 border-t border-white/[0.06]">
        <div className="flex items-center gap-3 mb-3">
          <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${ROLE_GRADIENT[user.role] ?? "from-indigo-500 to-violet-500"} flex items-center justify-center text-xs font-bold text-white shrink-0`}>
            {initials}
          </div>
        ...
        <button
          onClick={logout}
          className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs text-white/40 hover:text-rose-400 hover:bg-rose-500/10 transition-all duration-200 cursor-pointer"
        >
